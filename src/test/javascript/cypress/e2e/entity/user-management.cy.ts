...
describe('UserManagement e2e test', () => {
  ...
  let username: string;
  let password: string;
  const userManagementSample = { login: 'Donny.Jast16', email: 'Tamara0@yahoo.com' };
  ...
  describe.skip('new UserManagement page', () => {
    ...
    it('should create an instance of UserManagement', () => {
      cy.get(`[data-cy="login"]`).type('Meta.Tremblay97');
      cy.get(`[data-cy="login"]`).should('have.value', 'Meta.Tremblay97');

      cy.get(`[data-cy="firstName"]`).type('Colin');
      cy.get(`[data-cy="firstName"]`).should('have.value', 'Colin');

      cy.get(`[data-cy="lastName"]`).type('Upton');
      cy.get(`[data-cy="lastName"]`).should('have.value', 'Upton');

      cy.get(`[data-cy="email"]`).type('Brad31@gmail.com');
      cy.get(`[data-cy="email"]`).should('have.value', 'Brad31@gmail.com');
